...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased bg-gray-100">
        <div class="flex h-screen">
            <aside class="w-64 bg-white text-gray-800 shadow-lg relative flex flex-col border-r border-gray-200 h-screen">
                <div class="p-6">
                ...
                <nav class="mt-4 overflow-y-auto" style="max-height: calc(100vh - 180px);">
                    ...
                    <div class="mt-6">
                        <p class="px-6 text-xs font-semibold uppercase tracking-wide" style="color: #fab8c7;">Movimientos</p>
                        <a href="{{ route('entradas-materia-prima.index') }}" class="flex items-center space-x-3 px-6 py-2 mt-2 hover:bg-gray-50 transition-colors @if(request()->routeIs('entradas-materia-prima.*')) bg-gray-50 @endif">
                            <svg class="w-4 h-4" style="color: #4DC9C2;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                            </svg>
                            <span class="text-sm text-gray-700">Entrada Materias Primas</span>
                        </a>
                        @if(\Illuminate\Support\Facades\Route::has('salidas-materia-prima.index'))
                        <a href="{{ route('salidas-materia-prima.index') }}" class="flex items-center space-x-3 px-6 py-2 mt-1 hover:bg-gray-50 transition-colors @if(request()->routeIs('salidas-materia-prima.*')) bg-gray-50 @endif">
                            <svg class="w-4 h-4" style="color: #4DC9C2;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"></path>
                            </svg>
                            <span class="text-sm text-gray-700">Salida Materias Primas</span>
                        </a>
                        @endif
                        @if(\Illuminate\Support\Facades\Route::has('salidas-productos.index'))
                        <a href="{{ route('salidas-productos.index') }}" class="flex items-center space-x-3 px-6 py-2 mt-1 hover:bg-gray-50 transition-colors @if(request()->routeIs('salidas-productos.*')) bg-gray-50 @endif">
                            <svg class="w-4 h-4" style="color: #4DC9C2;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14M12 5l7 7-7 7"></path>
                            </svg>
                            <span class="text-sm text-gray-700">Salida Productos</span>
                        </a>
                        @endif
                    </div>
                    ...
                    <div class="mt-6 border-t border-gray-100 pt-4">
                        ...
                        <a href="{{ route('roles.management') }}" class="flex items-center space-x-3 px-6 py-2 mt-2 hover:bg-gray-50 transition-colors @if(request()->routeIs('users.index')) bg-gray-50 @endif">
                            <svg class="w-4 h-4" style="color: #4DC9C2;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path>
                            </svg>
                            <span class="text-sm text-gray-700 font-medium">Gestión de Roles</span>
                        </a>
                    </div>
                </nav>
