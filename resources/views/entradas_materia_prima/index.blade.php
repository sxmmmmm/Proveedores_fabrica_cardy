<x-with-sidebar-layout>
    <x-slot name="pageTitle">Historial — Entradas de Materia Prima</x-slot>
    ...
    <div x-data="{ open: {{ $errors->any() ? 'true' : 'false' }} }">
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">Historial de Entradas</h2>
                <p class="text-sm text-gray-500 mt-1">Trazabilidad completa de entradas de materia prima al inventario.</p>
            </div>
        ...
        <div x-show="open" x-cloak
            ...
            <div @click.stop
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                    <div>
                        <h3 class="text-lg font-bold text-gray-900">Registrar Nueva Entrada</h3>
                ...
                <form action="{{ route('entradas-materia-prima.store') }}" method="POST" class="p-6 space-y-4">
                    @csrf
    ...
    {{-- Filtros --}}
    <form method="GET" action="{{ route('entradas-materia-prima.index') }}"
    ...
    <div class="text-xs text-gray-500 mb-2">
        Mostrando {{ $entradas->firstItem() ?? 0 }}–{{ $entradas->lastItem() ?? 0 }}
        de {{ $entradas->total() }} registros
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($entradas as $entrada)
                    <tr onmouseover="this.style.backgroundColor='#E6F9F8'"
                            ...
                            @if($entrada->user)
                                <span class="inline-flex items-center gap-1.5">
                                    <span class="w-6 h-6 rounded-full text-white text-xs flex items-center justify-center font-bold flex-shrink-0"
                                          style="background-color: #14B8A6;">
                                        {{ strtoupper(substr($entrada->user->name, 0, 1)) }}
                                    </span>
                                    <span class="text-gray-700">{{ $entrada->user->name }}</span>
                                </span>
                            @else
                                <span class="text-gray-400 italic text-xs">{{ $entrada->usuario_nombre ?? 'N/A' }}</span>
                            @endif
                        </td>
                        <td class="px-4 py-3 font-medium text-gray-900">{{ $entrada->materiaPrima->nombre ?? 'N/A' }}</td>
                        <td class="px-4 py-3 font-bold text-green-700">+{{ $entrada->cantidad }}</td>
                        <td class="px-4 py-3 text-gray-600">{{ $entrada->fecha->format('d/m/Y') }}</td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3 text-gray-500 text-xs">{{ $entrada->observacion ?? '—' }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-10 text-center text-gray-400">No hay entradas registradas</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        @if($entradas->hasPages())
            <div class="px-4 py-3 border-t border-gray-100 bg-gray-50">{{ $entradas->links() }}</div>
        @endif
    </div>
</x-with-sidebar-layout>
